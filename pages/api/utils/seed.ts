...
const handleTeamsETL = async () => {
	...
	while (true) {
		...
		console.log(`Parsing Teams page ${pageNum++}. Found ${teamsRequest.data.length} Teams. [${teamsRequest.data[0].team_number}/${teamsRequest.data[teamsRequest.data.length - 1].team_number}]`)
...
const addTeamsToEvents = async () => {
	...
	for (const [index, eventPromise] of chunkedPromises.entries()) {
		const allEvents = await Promise.all(eventPromise.map((fn: Function) => fn()))
		console.log(`Event Chunk: ${index + 1}/${chunkedPromises.length}. Added the following events: ${allEvents.filter(event => event !== undefined).join(', ')}`)
	}
